...
help() {
	echo "		--bar <num>      - num bar"
	echo "		--after <text>   - put new bar after bar"
	echo "		--text <text>    - name bar"
	echo "		--icon <text>    - text icon"
	echo "		--ct <#XXXXXX>   - color text"
	echo "		--ci <#XXXXXX>   - color icon" 
	echo "		--e272 <command for left button click>"
	echo "		--e273 <command for right button>"
	echo "		--e768 <command for wheel up>"
	echo "		--e769 <command for wheel down>"
	echo "		-h,--help       - here"
	echo "		-s,--store      - save current bar(s)"
	echo "		-r,--restore    - restore bar from ~//store"
	echo "		-q              - silent mode for --get-id"
	echo "		--scratch_notes - put info for scratch"
	echo "		--get_id <text> - get id from name bar"
	echo "		--refresh       - refresh bar"
	echo "		--rename <text> - rename bar ( use --text <new name> )"
	echo "		--delete <text> - delete bar"
}
...
ff_get_id_bar() {
	...
	for (( i=1;i<=$(cat /tmp/swaybar/idb);i++ )); do
		if [ "$(cat /tmp/swaybar/bar-$i/full-text)" = "$1" ]; then
			xid_b=$i
			if [ $scr_note ] && [ -e /tmp/swaybar/bar-$i/about-bar ]; then
				cp /tmp/swaybar/bar-$i/about-bar /tmp/swaybar/scratch
			fi
			break
		fi
	done
...
if ! options=$(getopt -o qhsr -l help,scratch_notes,store,restore,refresh,rename:,get_id:,delete:,bar:,after:,ct:,ci:,text:,icon:,e272:,e273:,e768:,e769: -- "$@" 2>/dev/null); then
